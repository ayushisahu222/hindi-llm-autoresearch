...
step = 0
t_compiled = None
loss_history = []  # (elapsed_seconds, smoothed_loss)
...
while True:
    ...
    loss_history.append((total_training_time, debiased_smooth_loss))
...
chart_path = os.path.join(os.path.expanduser("~"), ".cache", "autoresearch", "loss_curve.png")
times = [t for t, _ in loss_history]
losses = [l for _, l in loss_history]
fig, ax = plt.subplots(figsize=(10, 4))
ax.plot(times, losses, linewidth=1.5, color="#2563eb")
ax.set_xlabel("Training time (seconds)")
ax.set_ylabel("Loss (smoothed)")
ax.set_title(f"Training Loss — final val_bpb: {val_bpb:.4f}")
...
model_save_path = os.path.join(os.path.expanduser("~"), ".cache", "autoresearch", "model.npz")
model._mask_cache = {}
model.save_weights(model_save_path)
import json
config_save_path = os.path.join(os.path.expanduser("~"), ".cache", "autoresearch", "config.json")
